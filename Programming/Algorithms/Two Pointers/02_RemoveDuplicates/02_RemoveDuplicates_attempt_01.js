...
function removeDuplicates(nums) {
    ...
    for(let i = 1; i< nums.length; i++) {
        // if nums[i] differes from nums[uniqueIndex] then move up one spot in the array and put the value at nums[i] in the nums[uniqueIndex] position.
        if (nums[i] !== nums[uniqueIndex]) {
            uniqueIndex++;
            nums[uniqueIndex] = nums[i];
        }

        // if they are the same just let it continue
    }
